...
submissionsRouter.post('/', turnstileMiddleware, rateLimitMiddleware, async (c) => {
  ...
  const body: Record<string, unknown> =
    ((c.get as (key: string) => unknown)('parsedBody') as Record<string, unknown>) ??
  ...
  try {
    ...
    const obj =
      (await env.MEDIA.get(fullKey)) ??
      (await env.MEDIA.get(`${imageResult.r2KeyPrefix}/full.webp`));
    if (obj) {
