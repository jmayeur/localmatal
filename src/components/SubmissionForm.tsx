...
function parseExifGps(buffer: ArrayBuffer, tiffStart: number): { lat: number; lng: number } | null {
  try {
    ...
    let latRef = 'N',
      lngRef = 'E';
    let latR: number[] | null = null,
      lngR: number[] | null = null;
...
function readRationals(v: DataView, off: number, count: number, le: boolean): number[] {
  ...
  for (let i = 0; i < count; i++) {
    const n = v.getUint32(off + i * 8, le),
      d = v.getUint32(off + i * 8 + 4, le);
    out.push(d === 0 ? 0 : n / d);
  }
  return out;
}
  ...
  return (
    <form class="submission-form" onSubmit={handleSubmit} noValidate>
      ...
      <div
        ...
        aria-label="Progress"
        style="font-family:var(--font-sans);font-size:var(--text-xs);color:var(--text-muted);margin-bottom:var(--sp-5)"
      >
        {(['photo', 'details', 'map', 'review'] as FormStep[]).map((s, i) => (
          <span
            key={s}
            style={`font-weight:${step === s ? '700' : '400'};margin-right:var(--sp-3)`}
          >
        ...
        <section aria-label="Step 2: About this place">
          <div class="details-header">
            ...
            {previewUrl && (
              <button
                type="button"
                class="photo-thumb-btn photo-thumb-btn--sm"
                onClick={() => setLightboxOpen(true)}
                aria-label="View uploaded photo"
              >
                <img
                  ...
                  class="photo-thumb photo-thumb--sm"
                />
              </button>
            )}
          </div>
          ...
          {submitState === 'nudge' && (
            <div class="nudge" role="alert">
              <p>
                The current place says: <em>"{nudgeSentence}"</em>
              </p>
          ...
          <div class="form-group">
            ...
            <input
              ...
              onInput={(e) =>
                setValues((v) => ({ ...v, place_name: (e.target as HTMLInputElement).value }))
              }
            />
          </div>

          <div class="form-group">
            <label class="form-label" for="contributor_name">
            ...
            <input
              ...
              onInput={(e) =>
                setValues((v) => ({ ...v, contributor_name: (e.target as HTMLInputElement).value }))
              }
            />
          </div>
          ...
          <div class="form-group">
            ...
            <textarea
              ...
              onInput={(e) =>
                setValues((v) => ({ ...v, sentence: (e.target as HTMLTextAreaElement).value }))
              }
          ...
          <div class="step-nav">
            <button type="button" class="btn btn-secondary" onClick={() => setStep('photo')}>
              ← Back
            </button>
            <button
              type="button"
              ...
              onClick={() => setStep('map')}
              disabled={
                !values.place_name || !values.contributor_name || senLen < 1 || senLen > 250
              }
            >
              Next →
            </button>
      ...
      {step === 'map' && (
        <section aria-label="Step 3: Place on map">
          ...
          />
          <div class="step-nav">
            <button type="button" class="btn btn-secondary" onClick={() => setStep('details')}>
              ← Back
            </button>
            <button
              type="button"
      ...
      {step === 'review' && (
        <section aria-label="Step 4: Review and submit">
          ...
          <dl style="font-family:var(--font-sans);font-size:var(--text-sm);display:grid;grid-template-columns:auto 1fr;gap:var(--sp-1) var(--sp-4);margin:0 0 var(--sp-6)">
            ...
            <dd style="margin:0">{values.place_name}</dd>
            <dt style="color:var(--text-muted)">Your name</dt>{' '}
            <dd style="margin:0">{values.contributor_name}</dd>
            <dt style="color:var(--text-muted)">Sentence</dt>{' '}
            <dd style="margin:0">
              <em>"{values.sentence}"</em>
            </dd>
          </dl>
          ...
          <div class="step-nav">
            <button type="button" class="btn btn-secondary" onClick={() => setStep('map')}>
              ← Back
            </button>
